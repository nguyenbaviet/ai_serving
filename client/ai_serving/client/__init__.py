...
__all__ = ['__version__', 'AIClient', 'ConcurrentAIClient']
...
class AIClient(object):
    def __init__(self, ip='localhost', port=5555, port_out=5556,
                 ...
                 check_version=False, check_length=False,
                 ignore_all_checks=False,
                 timeout=1*60*1000): # 4*60*1000 timeout after 4m, default is -1, mean forever

        """ A client object connected to a AIServer

        Create a AIClient that connects to a AIServer.
        ...
        if not ignore_all_checks and (check_version or show_server_config or check_length):
            if check_version and s_status['server_version'] != self.status['client_version']:
                raise AttributeError('version mismatch! server version is %s but client version is %s!\n'
                                     'consider "pip install -U ai-serving-server ai-serving-client"\n'
                                     'or disable version-check by "AIClient(check_version=False)"' % (
    ...
    def close(self):
        """
            Gently close all connections of the client. If you are using AIClient as context manager,
    ...
    def status(self):
        """
            Get the status of this AIClient instance

        :rtype: dict[str, str]
        :return: a dictionary contains the status of this AIClient instance
...
class ConcurrentAIClient(AIClient):
    def __init__(self, max_concurrency=10, retry=0, retry_gap=0.05, **kwargs):
        """ A thread-safe client object connected to a AIServer
        ...
        try:
            ...
            raise ImportError('AIClient module is not available, it is required for serving HTTP requests.'
                              'Please use "pip install -U ai-serving-client" to install it.'
                              'If you do not want to use it as an HTTP server, '
                              'then remove "-http_port" from the command line.')

        self.available_bc = [AIClient(**kwargs) for _ in range(max_concurrency)]
    ...
    def fetch(self, **kwargs):
        raise NotImplementedError('Async encoding of "ConcurrentAIClient" is not implemented yet')
